...
float4 main(PS_INPUT input) : SV_TARGET{
	...
	if (gDirLightCount > 0) {
		...
		ComputeDirectionalLight(gMaterial, gDirLights[0], input.normal, toEye, Am, Di, Sp);
	...
	if (gPointLightCount > 0) {
		...
		for (int i = 0; i < gPointLightCount; ++i) {
			...
			ComputePointLight(gMaterial, gPointLights[i], input.worldPos, input.normal, toEye, A, D, S);
			ambient += A; diffuse += D; spec += S;
			//ambient += A; diffuse += (shadow + A) * D; spec += (shadow + A) * S;
		}
	...
	if (gSpotLightCount > 0) {
		...
		for (int i = 0; i < gSpotLightCount; ++i) {
			...
			ComputeSpotLight(gMaterial, gSpotLights[i], input.worldPos, input.normal, toEye, A, D, S);
			ambient += A; diffuse += D; spec += S;
			//ambient += A; diffuse += (shadow + A) * D; spec += (shadow + A) * S;
		}
	...
	float alphal = log10(alpha);
	float alphall = 1.0f + alphal;
	litColor *= alphall;*/
	...
	//float alpha = texColor.a;
	//float alphal = alpha;
	//litColor.a = gMaterial.Diffuse.a * (1.0 + alphal);
	//litColor.a = gMaterial.Diffuse.a * texColor.a;
	litColor.a = gMaterial.Diffuse.a * alphae;
